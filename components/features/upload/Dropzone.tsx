...
/**
 * A dropzone component for handling file uploads.
 * @param param0 
 * @returns A React component that provides a drag-and-drop area for file uploads, as well as a clickable area to select files. It uses the react-dropzone library to manage file selection and drag-and-drop functionality, and it calls the onFileSelect callback with the selected file or null if there are any rejections.
 */
export default function Dropzone({ onFileSelect }: DropzoneProps) {
  ...
  return (
    <div className="w-3xl mx-auto mb-5">
      <div
        {...getRootProps()}
        className={`flex flex-col items-center justify-center w-full h-64 border-[1.5] border-dashed rounded-lg cursor-pointer transition-colors ${isDragActive
          ? 'border-primary bg-primary/10 hover:bg-primary/20 hover:border-primary'
          : 'border-primary bg-white hover:bg-base-200 hover:border-primary'
          }`}
      >
        <input {...getInputProps()} />
