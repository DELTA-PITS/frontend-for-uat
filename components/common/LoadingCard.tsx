import React from 'react'

interface LoadingCardProps {
  /** The primary loading message. Defaults to 'Loading...' */
  title?: string
  /** The secondary/subtext loading message. Defaults to 'Please wait while we process your document...' */
  subtitle?: string
  /** Additional CSS class names for custom container styling */
  className?: string
}

/**
 * Displays a loading state dialog with an animated spinner, title, and subtitle.
 * @param title - The primary loading message. Defaults to 'Loading...'
 * @param subtitle - The secondary/subtext loading message. Defaults to 'Please wait while we process your document...'
 * @param className - Additional CSS class names for custom container styling
 * @returns The loading card.
 */
const LoadingCard: React.FC<LoadingCardProps> = ({
  title = 'Loading...',
  subtitle = 'Please wait while we process your document...',
  className = '',
}) => {
  return (
    <div className={`bg-card rounded-xl shadow-lg p-6 sm:p-8 w-full max-w-3xl min-h-[17.5rem] flex flex-col justify-center mx-auto my-auto ${className}`}>
      <div className="flex flex-col items-center gap-4">
        <div className="w-20 h-20 flex items-center justify-center shrink-0">
          <div className="w-14 h-14 border-4 border-base-300 border-t-primary rounded-full animate-spin" />
        </div>

        <h3 className="text-2xl font-semibold text-base-content">{title}</h3>
        <p className="text-sm text-base-content/70 text-center">{subtitle}</p>
      </div>
    </div>
  )
}

export default LoadingCard
