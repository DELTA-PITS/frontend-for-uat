...
| `PageHero` | `components/common/PageHero.tsx` | **Baru 2026-08-02** — hero generik (badge + ikon + judul + subtitle + meta opsional), prop `accent` (`primary`/`secondary`). Diekstrak dari `VerifyHero`/`RegisterHero` yang tadinya 2 file JSX identik disinkronkan manual; sekarang keduanya cuma thin wrapper yang lempar copy + ikon ke `PageHero` |
| `InfoListCard` | `components/common/InfoListCard.tsx` | **Baru 2026-08-02** — card heading + bullet list, prop `headingIcon?`, `columns` (1/2), `bullet` (`check`/`dot`). Diekstrak dari `TipsCard`/`RequirementCard` yang tadinya duplikat struktur nyaris identik |

**Pola hero terunifikasi (sekarang komponen sungguhan, bukan konvensi manual)**: `VerifyHero.tsx` dan `RegisterHero.tsx` sama-sama cuma lempar copy + ikon + `accent` ke `PageHero` — badge kecil dengan ikon inline (BUKAN lagi avatar lingkaran besar `h-16 w-16`, dihapus 2026-08-01 karena Verify/Register adalah halaman workflow yang dilewati cepat, bukan landing page untuk berlama-lama), judul `text-2xl sm:text-3xl` (Page Title, kecil di mobile), gradient tipis `from-base-100 to-primary/5` atau `to-base-200`, rata kiri, padding `py-8 sm:py-10` (dipadatkan dari `py-12`) — hanya beda warna aksen (primary untuk Verify, secondary untuk Register) dan copy. Sebelum 2026-08-02 ini 2 file JSX terpisah yang harus disamakan manual (ada komentar eksplisit "jangan ubah salah satu tanpa menyamakan yang lain") — sekarang perubahan struktur cukup di `PageHero.tsx` sekali.

**⚠️ Gotcha daisyUI — `OperationCard` dan `.card-body p { flex-grow: 1 }`**: `OperationCard.tsx` pakai class asli daisyUI `card`/`card-body`. daisyUI diam-diam nge-set `flex-grow: 1` ke SETIAP `<p>` di dalam `.card-body`, di mana pun kedalamannya. Ini nggak kelihatan efeknya kecuali `<p>` itu juga jadi flex-item di container `flex-col` miliknya sendiri (persis kasus `Dropzone` — teksnya melar aneh, ada gap kosong besar antar baris). Fix permanen: `card-body` div di `OperationCard.tsx` dikasih `[&_p]:grow-0` supaya semua `<p>` di dalamnya otomatis `flex-grow: 0` lagi — komponen baru yang nested di `OperationCard` tidak perlu tahu soal ini lagi.
...
| Verify: section band ditambah — Hero (gradient), Form+Tips (`bg-base-100`, putih, menonjol dari page bg abu di sekitarnya), Cara Kerja Verifikasi (`bg-base-200`, senada dengan page bg default), Pertanyaan Umum (`bg-base-100`, putih lagi) — 4 band bergantian putih/abu, bukan 3 | Sebelumnya semua section menyatu di background yang sama, sulit membedakan batas antar section. **Catatan**: setelah revisi Design Language v2 (base-100/200 tukar peran, lihat §2), mekanismenya berbalik — `bg-base-100` sekarang yang "menonjol" (putih), bukan `bg-base-200` — tapi class di kode tidak berubah, cuma makna tokennya |
| Cara Kerja Verifikasi + Pertanyaan Umum: dulu satu `<section>` (satu band abu untuk berdua) → dipisah jadi 2 `<section>` sendiri-sendiri, bergantian abu/putih | Dua section dengan topik berbeda menyatu di band yang sama, tidak terasa sebagai section terpisah saat di-scroll |
