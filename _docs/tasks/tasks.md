...
| — | Audit sesi login (`session-auth-audit-2026-08-02.md`) diimplementasikan: `app/publisher` & `app/dashboard` diberi auth guard eksplisit (`redirect()`) + `dynamic = 'force-dynamic'`; `BfcacheRefresh` (listener `pageshow`) ditambah global untuk cegah halaman privat "muncul lagi" dari back/forward cache setelah logout; redirect target proxy.ts diarahkan ke `/` (`pages.signIn` di `auth.ts`); banner konfirmasi "Kamu berhasil keluar" ditambah setelah logout. **Koreksi penting**: ditemukan project sudah pakai Next.js 16 dengan `proxy.ts` (bukan `middleware.ts`) yang sudah aktif sejak awal — temuan awal "middleware tidak ada" di audit keliru, akar masalah sebenarnya murni bfcache | 2026-08-02 |
| 22 | Perbaiki konfigurasi Keycloak: daftarkan `http://localhost:3000/api/auth/logout` di "Valid Post Logout Redirect URIs" client di admin console — logout sebelumnya gagal dengan error "Invalid redirect uri" | 2026-08-02 |
| 2b | Validasi upload di **server** ditambahkan: `parseUpload.ts` sekarang mengembalikan `ParseUploadResult` diskriminatif dan menolak file >20MB (413) atau bukan PDF (415), bukan cuma cek `file instanceof File`. Diterapkan di `register`/`verify` route | 2026-08-02 |
| 3 | Setup test dibersihkan: `jest.config.ts` (nol test, environment salah) dihapus, `package.json#test` diarahkan ke `vitest run --project unit`, project `unit` (jsdom) ditambah di `vitest.config.ts` lengkap dengan alias `@lib`/`@components`/dst. 30 unit test baru ditulis untuk `lib/*.ts` (resultPayload, dateFormat, fileExtension, fileSizeCalc, uploadErrorMessage), `lib/i18n/translations.ts` (parity key ID/EN), dan `hooks/useUpload.ts` (alur sukses/gagal/no-file) | 2026-08-02 |
| 6 | Proteksi API diperkuat: pengecekan Keycloak access token yang terduplikasi di `register`/`records` route diekstrak jadi `app/api/_lib/requireAuth.ts` — satu sumber kebenaran, bukan lewat middleware/proxy karena redirect middleware akan merusak kontrak JSON API. Ditemukan sekaligus: `app/api/records/route.ts` ternyata **tidak dipakai** (`app/dashboard/page.tsx` fetch backend langsung, bukan lewat route ini) — dibiarkan untuk konsistensi, bukan dihapus (di luar scope task ini) | 2026-08-02 |
| 7 | Payload hasil (hash, record ID, transaction hash) dipindah dari query string ke `sessionStorage` (`lib/resultPayload.ts`) — href jadi `/result/<status>` polos, tidak ada lagi data sensitif di URL/history/log server. `ResultView.tsx` baca lewat `useEffect` (bukan `useMemo`) supaya tidak hydration-mismatch dengan SSR | 2026-08-02 |
| 9b | Dependency `lucide-react` (0 import) dihapus dari `package.json` | 2026-08-02 |
